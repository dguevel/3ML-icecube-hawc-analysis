...
p = argparse.ArgumentParser(description="Do signal trial for a declination",
...
if extension == 0:
    if index == 2:
        ...
        surfixextension = "_"+str(index)
else:
    if index == 2:
        surfixextension = "_"+str(extension)
    else:
        surfixextension = "_"+str(extension)+"_"+str(index)

#injection_spectrum = spectral.PowerLaw(1e3,1e-14,-index)
...
#if (dec >= -5 and index==3) or (extension > 0 and dec >= -5):
#    n_sigs = np.r_[25:72:5]
#elif (dec == -5 and index ==3) or (extension > 0 and dec >= -5):
#    n_sigs = np.r_[15:62:5]
#elif index > 2:
#    n_sigs = np.r_[2:10:2, 10:46.1:2]
#else:
#    n_sigs = np.r_[2:10:1, 10:30.1:2]
if args.dataset == "estes":
    n_sigs = np.r_[1:11:1]
else:
    n_sigs = np.r_[50:301:25]

folder="/Signal"+surfixextension
try:
    print(wrkdir+folder)
    ...
    pass
for n_sig in n_sigs:
    if os.path.exists(wrkdir+folder+"/SignalTrial_dec"+str(dec)+"_ninj_"+str(n_sig)+"index_"+str(index)+"_" + surfix+".npy"):
        if args.overwrite:
            os.remove(wrkdir+folder+"/SignalTrial_dec"+str(dec)+"_ninj_"+str(n_sig)+"index_"+str(index)+"_" + surfix+".npy")
        else:
            continue
    ...
    if analysislist.newton_flux_norm:
        ...
        my_grid = {'TXS.spectrum.main.Powerlaw.index': np.linspace(-2, -3.99, 2)}

    grid_minimizer.setup(
        second_minimization=local_minimizer, grid=my_grid)
    for ntrial in range(nscramble):
        analysislist.injection(n_signal=n_sig,poisson=True)
    ...
    np.save(wrkdir+folder+"/SignalTrial_dec"+str(dec)+surfixextension+"_ninj_"+str(n_sig)+"index_"+str(index)+"_" + surfix+".npy",result)